...
import { Users, BookOpen, Plus, TrendingUp, Shield, MoreVertical, Check, X, Trash2, AlertCircle, Loader2 } from 'lucide-react';
...
export default function AdminDashboard() {
  const [activeTab, setActiveTab] = useState<'approvals' | 'lessons'>('approvals');
  ...
  const handleAddLesson = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        ...
        .insert([newLesson]);
      if (error) throw error;
      setNewLesson({ title: '', video_url: '', materials_link: '', tier: 'Beginner', description: '' });
                  ...
                  {pendingUsers.map((u) => (
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-[#2D3436]">{u.email}</p>
                          <p className="text-[10px] text-[#2D3436]/40 uppercase font-black tracking-widest">{u.role}</p>
                        </div>
                      </div>
              ...
              <div className="lg:col-span-8 space-y-8">
                <div className="bg-white border border-[#D9C5A0]/30 rounded-[40px] p-10 shadow-ambient space-y-10">
                  ...
                  <form onSubmit={handleAddLesson} className="space-y-6">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-[#2D3436] uppercase tracking-widest pl-1">Video URL</label>
                        <input 
                          type="url" 
                          required
                      ...
                      <div className="space-y-2">
                        <label className="text-xs font-bold text-[#2D3436] uppercase tracking-widest pl-1">PSD Resource Link</label>
                        <input 
                          type="text" 
                          value={newLesson.materials_link}
                          onChange={(e) => setNewLesson({...newLesson, materials_link: e.target.value})}
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-[#2D3436] uppercase tracking-widest pl-1">Description</label>
                      <textarea 
                        rows={4}
                        required
